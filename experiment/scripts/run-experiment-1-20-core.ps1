...
. (Join-Path $PSScriptRoot 'experiment-1-20-artifact-contract.ps1')
...
$id="RUN-$RunDate-EXP120-$Condition-$RunIndex"; $warm="SMOKE-$RunDate-EXP120-$Condition-$RunIndex"
$result=Join-Path $root 'backend\experiments\results\experiment-1-20'; $target=Join-Path $result "core\$id"; $staging=Join-Path $result "collector-staging\$id"; $legacy=Join-Path $root "experiment\results\$id"; $plan=Join-Path $result "plan\$Condition-$RunIndex-command-manifest.json"
$files=@('backend\docker-compose.experiment.yaml','backend\docker-compose.app-instance-t3-medium.yaml','backend\docker-compose.mock-headroom-4cpu.yaml','backend\docker-compose.http-pool-400.yaml','backend\docker-compose.mock-memory-headroom.yaml','backend\docker-compose.experiment-1-12-connection-attribution.yaml','backend\docker-compose.experiment-1-19-fresh-first-lifecycle.yaml'); $args=@(); foreach($f in $files){$args+=@('-f',(Join-Path $root $f))}; $project='doeng-exp120-core'
...
try {
    ...
    $ready=Wait-Exp120CollectorReady -Process $collector -JsonlPath (Join-Path $staging 'pool-metrics.jsonl') -TimeoutSeconds 10; $steps.Add('collector readiness')
    ...
    $container=(docker compose -p $project @args ps -q flux-corrected).Trim(); if([string]::IsNullOrWhiteSpace($container)){throw 'Application container unavailable for existing log artifact capture'}; New-Item -ItemType Directory -Force -Path $legacy|Out-Null; docker logs --timestamps $container 2>&1|ForEach-Object{$_.ToString()}|Set-Content (Join-Path $legacy 'application.log') -Encoding UTF8; if($LASTEXITCODE){throw 'Application log capture failed'}
...
if($null-ne$failure){
    ...
    if($collectorStartedAt){
        ...
        foreach($line in (Get-Exp120CollectorLines (Join-Path $staging 'pool-metrics.jsonl'))){try{$sample=($line -replace '^\uFEFF','')|ConvertFrom-Json -ErrorAction Stop;$at=ConvertTo-Exp120UtcIso $sample.timestamp;if($null-eq$sample.failure -or [string]::IsNullOrWhiteSpace([string]$sample.failure)){$valid++;if($null-eq$first){$first=$at};$last=$at}}catch{$invalid++}}
