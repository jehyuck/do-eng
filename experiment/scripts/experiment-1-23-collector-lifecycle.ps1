function ConvertTo-Exp123Timestamp([object]$Value) {
    $p=[DateTimeOffset]::MinValue
    if(-not [DateTimeOffset]::TryParse([string]$Value,[ref]$p)){throw "Invalid timestamp: $Value"}
    $p.ToUniversalTime()
}
function Wait-Exp123CollectorReady([System.Diagnostics.Process]$Process,[string]$JsonlPath,[int]$TimeoutSeconds=10) {
    ...
    while((Get-Date).ToUniversalTime()-lt$deadline){
        ...
        foreach($line in @(Get-Content $JsonlPath -ErrorAction SilentlyContinue|Where-Object{$_.Trim()})){
            try{$s=$line|ConvertFrom-Json;$at=ConvertTo-Exp123Timestamp $s.timestamp;if($null-eq$s.failure-or[string]::IsNullOrWhiteSpace([string]$s.failure)){return [ordered]@{collectorReadyAt=(Get-Date).ToUniversalTime().ToString('o');collectorFirstValidSampleAt=$at.ToString('o')}}}catch{}
        }
        Start-Sleep -Milliseconds 100
    }
    throw 'COLLECTOR_NOT_READY'
}
function Wait-Exp123CollectorCoversCoreEnd([string]$JsonlPath,[string]$CoreCompletedAt,[int]$TimeoutSeconds=10) {
