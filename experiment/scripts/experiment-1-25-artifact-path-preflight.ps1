function Get-Exp125RelativePath { param([string]$Base,[string]$Candidate); ([Uri]$Base).MakeRelativeUri([Uri]$Candidate).ToString() }
function Initialize-Exp125ArtifactDirectories {
...
function Test-Exp125ArtifactDirectoryWriteability {
 param([Parameter(Mandatory)][string]$RunRoot)
...
function Invoke-Exp125ArtifactPathPreflight {
 ...
 try { Initialize-Exp125ArtifactDirectories $RunRoot; $rows=@(Test-Exp125ArtifactDirectoryWriteability $RunRoot); if($rows.Count -ne 7){throw 'ARTIFACT_PATH_PREFLIGHT_DIRECTORY_COUNT_INVALID'};if(@($rows|Where-Object{$_.directoryExistsAfterInitialization -ne $true -or $_.pathContained -ne $true -or $_.writeProbeCreated -ne $true -or $_.writeProbeContentVerified -ne $true -or $_.deleteProbePassed -ne $true -or $_.probeFileAbsentAfterDelete -ne $true}).Count -gt 0){throw 'ARTIFACT_PATH_PREFLIGHT_PROBE_FAILED'} }
...
function Assert-Exp125ArtifactPathPreflight {
 param([Parameter(Mandatory)][string]$RunRoot)
