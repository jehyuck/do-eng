...
$firstActiveLimit = First-AtOrAbove $active 999.5
...
$contractValid = ($configuredMaxConnections -eq 1000 -and $configuredMaxPending -eq 800)
...
} elseif ($meterCoverageValid -and $peakActive -ge 999.5 -and $peakPending -gt 0 -and $exceptionCounts.poolAcquirePendingLimit -gt 0) {
    $classification = "POOL1000_RUNTIME_SATURATION_PROVEN"
} elseif ($meterCoverageValid -and $peakActive -lt 999.5) {
    $classification = "POOL1000_RUNTIME_NOT_SATURATED"
} else {
    $classification = "INSUFFICIENT_POOL1000_EVIDENCE"
}
...
$result = [ordered]@{
    ...
    interpretationGuard = @(
        ...
        "POOL1000_RUNTIME_SATURATION_PROVEN requires active-at-limit, positive pending, and PoolAcquirePendingLimitException together",
...
$markdown = @"
# Exp140 Pool1000 Intervention Summary
...
- First active >= 1000: `$($firstActiveLimit.timestamp)`
