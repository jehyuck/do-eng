...
$overrideCompose = Join-Path $repo "experiment\compose\experiment-1-45-pool500-pending1.override.yml"
...
$summarizer = Join-Path $repo "experiment\scripts\summarize-exp145-pool500-pending1.ps1"
$fixture = Join-Path $repo "image\arc.jpg"
$resultRoot = Join-Path $repo "backend\experiments\results\experiment-1-45\pool500-pending1-001"
$planDir = Join-Path $resultRoot "plan"
$warmupDir = Join-Path $resultRoot "WARMUP\WARMUP-EXP145-POOL500-PENDING1-001"
$highDir = Join-Path $resultRoot "HIGH\RUN-EXP145-POOL500-PENDING1-HIGH-001"
$analysisDir = Join-Path $resultRoot "analysis"
$runtimeCompose = $null
$runtimeDbDumpDir = $null
$runtimeSeedImage = $null
$project = "doeng-exp145"
...
function Prepare-RuntimeCompose {
    ...
    $script:runtimeDbDumpDir = Join-Path $env:TEMP "doeng-exp145-db-seed"
    ...
    $script:runtimeSeedImage = "doeng-exp145-mariadb-seeded:latest"
    ...
    $script:runtimeCompose = Join-Path $repo "backend\docker-compose.experiment.exp145-runtime.yaml"
...
if (-not (Test-Path -LiteralPath $overrideCompose)) { throw "Exp145 override not found" }
...
if (-not (Test-Path -LiteralPath $summarizer)) { throw "Exp145 summarizer not found" }
...
if ($dirty.Count -gt 0) {
    throw "Working tree must be clean. Use a dedicated clean worktree for Exp145."
}
...
Save-Json (Join-Path $planDir "baseline.json") ([ordered]@{
    ...
    question = "Does eliminating the pending acquire queue at maxConnections=500 change rejection, timeout, and successful RPS under HIGH load?"
})
...
$requiredPatterns = [ordered]@{
    ...
    maxPending = 'DOENG_SHARED_POOL_PENDING_MAX_COUNT:\s*"?1"?'
...
Save-Json (Join-Path $planDir "static-contract.json") ([ordered]@{
    ...
    maxPendingVariable = "DOENG_SHARED_POOL_PENDING_MAX_COUNT=1"
...
if ($ExecutionMode -eq "PREFLIGHT") {
    Write-Output "EXP145_STATIC_PREFLIGHT_PASS"
    exit 0
}
...
try {
    ...
    if ($runtimeContract.maxConnections -ne "500" -or $runtimeContract.maxPending -ne "1") { throw "Pool runtime env mismatch" }
    if ($runtimeContract.poolObservation -ne "true") { throw "Pool observation is not enabled" }

    $warmup = Invoke-NodeLoad -RunId "WARMUP-EXP145-POOL500-PENDING1-001" -ActiveMissions 1 `
    ...
    for ($attempt = 1; $attempt -le 10; $attempt++) {
        try {
            ...
            if ($null -ne $active -and $null -ne $pending -and $maxConnections -eq 500 -and $maxPending -eq 1) {
                $poolReady = [ordered]@{
                    status = "PASS"
                    attempt = $attempt
                    snapshot = $snapshot
    ...
    if ($null -eq $poolReady) {
        ...
        throw "EXP145_POOL500_PENDING1_METER_NOT_READY_AFTER_WARMUP"
    }
    ...
    $high = Invoke-NodeLoad -RunId "RUN-EXP145-POOL500-PENDING1-HIGH-001" -ActiveMissions 200 `
    ...
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP145_POOL500_PENDING1_DIAGNOSTIC_COMPLETE"
        warmupRunId = "WARMUP-EXP145-POOL500-PENDING1-001"
        highRunId = "RUN-EXP145-POOL500-PENDING1-HIGH-001"
        additionalRuns = 0
        pool500Pending1Executed = $true
        mvcExecuted = $false
        productionJavaModified = $false
    })
    Write-Output "EXP145_POOL500_PENDING1_DIAGNOSTIC_COMPLETE"
} catch {
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP145_EXECUTION_STOPPED"
