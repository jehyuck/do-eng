...
$overrideCompose = Join-Path $repo "experiment\compose\experiment-1-46-broad-attribution.override.yml"
...
$summarizer = Join-Path $repo "experiment\scripts\summarize-exp146-broad-attribution.ps1"
$fixture = Join-Path $repo "image\arc.jpg"
$resultRoot = Join-Path $repo "backend\experiments\results\experiment-1-46\broad-attribution-001"
$planDir = Join-Path $resultRoot "plan"
$warmupDir = Join-Path $resultRoot "WARMUP\WARMUP-EXP146-BROAD-001"
$highDir = Join-Path $resultRoot "HIGH\RUN-EXP146-BROAD-HIGH-001"
$analysisDir = Join-Path $resultRoot "analysis"
$runtimeCompose = $null
$runtimeDbDumpDir = $null
$runtimeSeedImage = $null
$project = "doeng-exp146"
...
function Prepare-RuntimeCompose {
    ...
    $script:runtimeDbDumpDir = Join-Path $env:TEMP "doeng-exp146-db-seed"
    ...
    $script:runtimeSeedImage = "doeng-exp146-mariadb-seeded:latest"
    ...
    $script:runtimeCompose = Join-Path $repo "backend\docker-compose.experiment.exp146-runtime.yaml"
...
if (-not (Test-Path -LiteralPath $overrideCompose)) { throw "Exp146 override not found" }
...
if (-not (Test-Path -LiteralPath $summarizer)) { throw "Exp146 summarizer not found" }
...
if ($dirty.Count -gt 0) {
    throw "Working tree must be clean. Use a dedicated clean worktree for Exp146."
}
...
$requiredPatterns = [ordered]@{
    ...
    stageObservation = 'DOENG_STAGE_OBSERVATION_ENABLED:\s*"?true"?'
...
if ($ExecutionMode -eq "PREFLIGHT") {
    Write-Output "EXP146_STATIC_PREFLIGHT_PASS"
    exit 0
}
...
try {
    ...
    if ($runtimeContract.stageObservation -ne "true") { throw "Stage observation is not enabled" }
    try {
        ...
        throw "EXP146_STAGE_ENDPOINT_INVALID: $($_.Exception.Message)"
    }

    $warmup = Invoke-NodeLoad -RunId "WARMUP-EXP146-BROAD-001" -ActiveMissions 1 `
    ...
    if ($null -eq $poolReady) {
        ...
        throw "EXP146_POOL500_METER_NOT_READY_AFTER_WARMUP"
    }
    ...
    $stageJob = Start-Job -ScriptBlock {
    ...
    } -ArgumentList $managementUrl, $stageTimeline
    ...
    $high = Invoke-NodeLoad -RunId "RUN-EXP146-BROAD-HIGH-001" -ActiveMissions 200 `
    ...
    & docker compose -p $project -f $baseCompose -f $overrideCompose exec -T mariadb mariadb -udoeng -pdoeng-experiment-pass doeng -e "SHOW ENGINE INNODB STATUS;" |
    ...
    & $summarizer -PoolTimelinePath $poolTimeline `
        ...
        -OutputJsonPath (Join-Path $analysisDir "exp146-attribution-summary.json") `
        -OutputMarkdownPath (Join-Path $analysisDir "exp146-attribution-report.md") |
    ...
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP146_BROAD_ATTRIBUTION_COMPLETE"
        warmupRunId = "WARMUP-EXP146-BROAD-001"
        highRunId = "RUN-EXP146-BROAD-HIGH-001"
        additionalRuns = 0
        broadAttributionExecuted = $true
        mvcExecuted = $false
        productionJavaModified = $false
    })
    Write-Output "EXP146_BROAD_ATTRIBUTION_COMPLETE"
} catch {
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP146_EXECUTION_STOPPED"
