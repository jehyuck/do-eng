...
$summarizer = Join-Path $repo "experiment\scripts\summarize-exp147-db-write-order-fix.ps1"
$fixture = Join-Path $repo "image\arc.jpg"
$resultRoot = Join-Path $repo "backend\experiments\results\experiment-1-47\db-write-order-fix-001"
$planDir = Join-Path $resultRoot "plan"
$warmupDir = Join-Path $resultRoot "WARMUP\WARMUP-EXP147-DB-WRITE-ORDER-001"
$highDir = Join-Path $resultRoot "HIGH\RUN-EXP147-DB-WRITE-ORDER-HIGH-001"
$analysisDir = Join-Path $resultRoot "analysis"
$runtimeCompose = $null
$runtimeDbDumpDir = $null
$runtimeSeedImage = $null
$project = "doeng-exp147"
...
function Prepare-RuntimeCompose {
    ...
    $script:runtimeDbDumpDir = Join-Path $env:TEMP "doeng-exp147-db-seed"
    ...
    $script:runtimeSeedImage = "doeng-exp147-mariadb-seeded:latest"
    ...
    $script:runtimeCompose = Join-Path $repo "backend\docker-compose.experiment.exp147-runtime.yaml"
...
if (-not (Test-Path -LiteralPath $summarizer)) { throw "Exp147 summarizer not found" }
...
try {
    ...
    try {
        ...
        throw "EXP147_STAGE_ENDPOINT_INVALID: $($_.Exception.Message)"
    }

    $warmup = Invoke-NodeLoad -RunId "WARMUP-EXP147-DB-WRITE-ORDER-001" -ActiveMissions 1 `
    ...
    if ($null -eq $poolReady) {
        ...
        throw "EXP147_POOL500_METER_NOT_READY_AFTER_WARMUP"
    }
    ...
    $high = Invoke-NodeLoad -RunId "RUN-EXP147-DB-WRITE-ORDER-HIGH-001" -ActiveMissions 200 `
    ...
    & $summarizer -PoolTimelinePath $poolTimeline `
        ...
        -OutputJsonPath (Join-Path $analysisDir "exp147-db-write-order-summary.json") `
        -OutputMarkdownPath (Join-Path $analysisDir "exp147-db-write-order-report.md") |
    ...
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP147_DB_WRITE_ORDER_COMPLETE"
        warmupRunId = "WARMUP-EXP147-DB-WRITE-ORDER-001"
        highRunId = "RUN-EXP147-DB-WRITE-ORDER-HIGH-001"
        additionalRuns = 0
        broadAttributionExecuted = $true
        mvcExecuted = $false
        productionJavaModified = $false
    })
    Write-Output "EXP147_DB_WRITE_ORDER_COMPLETE"
} catch {
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP147_EXECUTION_STOPPED"
