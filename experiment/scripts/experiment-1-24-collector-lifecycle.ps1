function ConvertTo-Exp124Timestamp([object]$Value) {
    $p=[DateTimeOffset]::MinValue
    if(-not [DateTimeOffset]::TryParse([string]$Value,[ref]$p)){throw "Invalid timestamp: $Value"}
    $p.ToUniversalTime()
}
function Wait-Exp124CollectorReady([System.Diagnostics.Process]$Process,[string]$JsonlPath,[int]$TimeoutSeconds=10) {
