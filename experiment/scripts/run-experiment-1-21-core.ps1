...
try {
    ...
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot 'run-isolated-vu-success-smoke.ps1') -RunId $id -Implementation "exp121-$Condition-core" -TargetUrl http://127.0.0.1:8001/game/face -ServerService flux-corrected -ComposeProject $project -ComposeFiles ($files -join ',') -ActiveMissions 200 -InitialActiveUsers 200 -ActivationStepUsers 200 -AiDelayMs 2000 -StorageDelayMs 100 -IntervalMs 1000 -DurationMs 105000 -RequestTimeoutMs 10000 -TargetP95Ms 10000 -EnableObservability 1 -EnableJfr 0 -EnableContainerMonitor 1 -SkipApplicationSnapshot 1 -SkipMockMetrics 1 -OutcomeMode controlled-admission -AccountingMode corrected -DrainObservationSeconds 30
...
if($null-eq$failure){try{Write-Json ([ordered]@{runId=$id;executionStatus='COMPLETED';artifactValidation='PASSED';completedSteps=@($steps);cleanupResult=$cleanup}) (Join-Path $target 'execution-summary.json');Assert-Exp121Artifacts $target;$timeline=Get-Content (Join-Path $target 'provenance/execution-timeline.json') -Raw|ConvertFrom-Json;$timeline.finalizationCompletedAt=(Get-Date).ToUniversalTime().ToString('o');Write-Json $timeline (Join-Path $target 'provenance/execution-timeline.json');Set-Exp121Terminal $target 'COMPLETED'}catch{$failure=$_}}
if($null-ne$failure){
