...
try {
  ...
  $v=Join-Path $artifact 'verification-summary.json';if(-not(Test-Path $v)){throw 'VERIFICATION_MISSING'};if(-not(Test-Path (Join-Path $artifact 'client-results.json')) -or -not(Test-Path (Join-Path $artifact 'mock-drain-summary.json')) -or -not(Test-Path (Join-Path $artifact 'db-after.json'))){throw 'PRIMARY_ARTIFACT_MISSING'}
