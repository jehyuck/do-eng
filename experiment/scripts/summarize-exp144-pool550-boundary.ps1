...
$firstActiveLimit = First-AtOrAbove $active 549.5
...
$contractValid = ($configuredMaxConnections -eq 550 -and $configuredMaxPending -eq 800)
...
} elseif ($meterCoverageValid -and $peakActive -ge 549.5 -and $peakPending -gt 0 -and $exceptionCounts.poolAcquirePendingLimit -gt 0) {
    $classification = "POOL550_RUNTIME_SATURATION_PROVEN"
} elseif ($meterCoverageValid -and $peakActive -lt 549.5) {
    $classification = "POOL550_RUNTIME_NOT_SATURATED"
} else {
    $classification = "INSUFFICIENT_POOL550_EVIDENCE"
}
...
$result = [ordered]@{
    ...
    interpretationGuard = @(
        ...
        "POOL550_RUNTIME_SATURATION_PROVEN requires active-at-limit, positive pending, and PoolAcquirePendingLimitException together",
...
$markdown = @"
# Exp144 Pool550 Boundary Probe Summary
...
- First active >= 550: `$($firstActiveLimit.timestamp)`
