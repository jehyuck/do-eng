param(
    [string]$OutputDirectory = ".experiment-work\experiment-1-13-controls"
)
...
$composeProject = "doeng-exp113-control"
$composeFiles = @(
...
$env:DOENG_EXTERNAL_MAX_IDLE_TIME_MS = "0"
...
if (Test-Path -LiteralPath $output) { throw "positive control output already exists: $output" }
...
try {
    ...
    if ($null -eq $previousLimit) { Remove-Item Env:DOENG_ADMISSION_MAX_CONCURRENT -ErrorAction SilentlyContinue } else { $env:DOENG_ADMISSION_MAX_CONCURRENT = $previousLimit }
    ...
    if ($null -eq $previousTarget) { Remove-Item Env:TARGET_URL -ErrorAction SilentlyContinue } else { $env:TARGET_URL = $previousTarget }
    if ($null -eq $previousMock) { Remove-Item Env:MOCK_URL -ErrorAction SilentlyContinue } else { $env:MOCK_URL = $previousMock }
    if ($null -eq $previousResult) { Remove-Item Env:RESULT_PATH -ErrorAction SilentlyContinue } else { $env:RESULT_PATH = $previousResult }
}
...
# The reused Experiment 1-12 control fixture emits EXP112-CONTROL in both
# application and mock events. Preserve that correlation contract here.
[ordered]@{ summary = [ordered]@{ experimentRunId = "EXP112-CONTROL" }; requests = $control.requests } |
    ConvertTo-Json -Depth 20 | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $output "client-results.json")
[ordered]@{ aiLifecycleEvents = $control.aiLifecycleEvents } |
