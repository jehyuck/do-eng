...
$summarizer = Join-Path $repo "experiment\scripts\summarize-exp139-pool-evidence.ps1"
$fixture = Join-Path $repo "image\arc.jpg"
$resultRoot = Join-Path $repo "backend\experiments\results\experiment-1-39\pool-evidence-recovery-001"
...
if (-not (Test-Path -LiteralPath $overrideCompose)) { throw "Exp139 override not found" }
...
if (-not (Test-Path -LiteralPath $summarizer)) { throw "Exp139 summarizer not found" }
...
if ($dirty.Count -gt 0) {
    throw "Working tree must be clean. Use a dedicated clean worktree for Exp139."
}
...
Save-Json (Join-Path $planDir "baseline.json") ([ordered]@{
    ...
    question = "Does the shared Reactor Netty pool reach maxConnections=400 with pending acquisition and direct pending-limit failure under HIGH load?"
})
...
$requiredPatterns = [ordered]@{
    ...
    maxConnections = 'DOENG_SHARED_POOL_MAX_CONNECTIONS:\s*"?400"?'
...
Save-Json (Join-Path $planDir "static-contract.json") ([ordered]@{
    status = "PASS"
    maxConnectionsVariable = "DOENG_SHARED_POOL_MAX_CONNECTIONS=400"
...
if ($ExecutionMode -eq "PREFLIGHT") {
    Write-Output "EXP139_STATIC_PREFLIGHT_PASS"
    exit 0
}
...
try {
    ...
    if ($runtimeContract.maxConnections -ne "400" -or $runtimeContract.maxPending -ne "800") { throw "Pool runtime env mismatch" }
    if ($runtimeContract.poolObservation -ne "true") { throw "Pool observation is not enabled" }

    $warmup = Invoke-NodeLoad -RunId "WARMUP-EXP139-POOL-METER-001" -ActiveMissions 1 `
    ...
    for ($attempt = 1; $attempt -le 10; $attempt++) {
        try {
            ...
            if ($null -ne $active -and $null -ne $pending -and $maxConnections -eq 400 -and $maxPending -eq 800) {
                $poolReady = [ordered]@{
                    status = "PASS"
                    attempt = $attempt
                    snapshot = $snapshot
    ...
    if ($null -eq $poolReady) {
        ...
        throw "EXP139_POOL_METER_NOT_READY_AFTER_WARMUP"
    }
    ...
    $high = Invoke-NodeLoad -RunId "RUN-EXP139-POOL400-HIGH-001" -ActiveMissions 200 `
    ...
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP139_POOL_EVIDENCE_COMPLETE"
        warmupRunId = "WARMUP-EXP139-POOL-METER-001"
        highRunId = "RUN-EXP139-POOL400-HIGH-001"
        additionalRuns = 0
        pool1000Executed = $false
        mvcExecuted = $false
        productionJavaModified = $false
    })
    Write-Output "EXP139_POOL_EVIDENCE_COMPLETE"
} catch {
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP139_EXECUTION_STOPPED"
