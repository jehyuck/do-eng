...
$overrideCompose = Join-Path $repo "experiment\compose\experiment-1-44-pool550-boundary.override.yml"
...
$summarizer = Join-Path $repo "experiment\scripts\summarize-exp144-pool550-boundary.ps1"
$fixture = Join-Path $repo "image\arc.jpg"
$resultRoot = Join-Path $repo "backend\experiments\results\experiment-1-44\pool550-boundary-001"
$planDir = Join-Path $resultRoot "plan"
$warmupDir = Join-Path $resultRoot "WARMUP\WARMUP-EXP144-POOL550-METER-001"
$highDir = Join-Path $resultRoot "HIGH\RUN-EXP144-POOL550-HIGH-001"
$analysisDir = Join-Path $resultRoot "analysis"
$runtimeCompose = $null
$runtimeDbDumpDir = $null
$runtimeSeedImage = $null
$project = "doeng-exp144"
...
function Prepare-RuntimeCompose {
    ...
    $script:runtimeDbDumpDir = Join-Path $env:TEMP "doeng-exp144-db-seed"
    ...
    $script:runtimeSeedImage = "doeng-exp144-mariadb-seeded:latest"
    ...
    $script:runtimeCompose = Join-Path $repo "backend\docker-compose.experiment.exp144-runtime.yaml"
...
if (-not (Test-Path -LiteralPath $overrideCompose)) { throw "Exp144 override not found" }
...
if (-not (Test-Path -LiteralPath $summarizer)) { throw "Exp144 summarizer not found" }
...
if ($dirty.Count -gt 0) {
    throw "Working tree must be clean. Use a dedicated clean worktree for Exp144."
}
...
Save-Json (Join-Path $planDir "baseline.json") ([ordered]@{
    ...
    question = "Does the shared Reactor Netty pool reach maxConnections=550 with pending acquisition and direct pending-limit failure under HIGH load?"
})
...
$requiredPatterns = [ordered]@{
    ...
    maxConnections = 'DOENG_SHARED_POOL_MAX_CONNECTIONS:\s*"?550"?'
...
Save-Json (Join-Path $planDir "static-contract.json") ([ordered]@{
    status = "PASS"
    maxConnectionsVariable = "DOENG_SHARED_POOL_MAX_CONNECTIONS=550"
...
if ($ExecutionMode -eq "PREFLIGHT") {
    Write-Output "EXP144_STATIC_PREFLIGHT_PASS"
    exit 0
}
...
try {
    ...
    if ($runtimeContract.maxConnections -ne "550" -or $runtimeContract.maxPending -ne "800") { throw "Pool runtime env mismatch" }
    if ($runtimeContract.poolObservation -ne "true") { throw "Pool observation is not enabled" }

    $warmup = Invoke-NodeLoad -RunId "WARMUP-EXP144-POOL550-METER-001" -ActiveMissions 1 `
    ...
    for ($attempt = 1; $attempt -le 10; $attempt++) {
        try {
            ...
            if ($null -ne $active -and $null -ne $pending -and $maxConnections -eq 550 -and $maxPending -eq 800) {
                $poolReady = [ordered]@{
                    status = "PASS"
                    attempt = $attempt
                    snapshot = $snapshot
    ...
    if ($null -eq $poolReady) {
        ...
        throw "EXP144_POOL550_METER_NOT_READY_AFTER_WARMUP"
    }
    ...
    $high = Invoke-NodeLoad -RunId "RUN-EXP144-POOL550-HIGH-001" -ActiveMissions 200 `
    ...
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP144_POOL550_PROBE_COMPLETE"
        warmupRunId = "WARMUP-EXP144-POOL550-METER-001"
        highRunId = "RUN-EXP144-POOL550-HIGH-001"
        additionalRuns = 0
        pool550Executed = $true
        mvcExecuted = $false
        productionJavaModified = $false
    })
    Write-Output "EXP144_POOL550_PROBE_COMPLETE"
} catch {
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP144_EXECUTION_STOPPED"
