...
$overrideCompose = Join-Path $repo "experiment\compose\experiment-1-42-pool500-probe.override.yml"
...
$summarizer = Join-Path $repo "experiment\scripts\summarize-exp142-pool500-probe.ps1"
$fixture = Join-Path $repo "image\arc.jpg"
$resultRoot = Join-Path $repo "backend\experiments\results\experiment-1-42\pool500-probe-001"
$planDir = Join-Path $resultRoot "plan"
$warmupDir = Join-Path $resultRoot "WARMUP\WARMUP-EXP142-POOL500-METER-001"
$highDir = Join-Path $resultRoot "HIGH\RUN-EXP142-POOL500-HIGH-001"
$analysisDir = Join-Path $resultRoot "analysis"
$runtimeCompose = $null
$runtimeDbDumpDir = $null
$runtimeSeedImage = $null
$project = "doeng-exp142"
...
function Prepare-RuntimeCompose {
    ...
    $script:runtimeDbDumpDir = Join-Path $env:TEMP "doeng-exp142-db-seed"
    ...
    $script:runtimeSeedImage = "doeng-exp142-mariadb-seeded:latest"
    ...
    $script:runtimeCompose = Join-Path $repo "backend\docker-compose.experiment.exp142-runtime.yaml"
...
if (-not (Test-Path -LiteralPath $overrideCompose)) { throw "Exp142 override not found" }
...
if (-not (Test-Path -LiteralPath $summarizer)) { throw "Exp142 summarizer not found" }
...
if ($dirty.Count -gt 0) {
    throw "Working tree must be clean. Use a dedicated clean worktree for Exp142."
}
...
Save-Json (Join-Path $planDir "baseline.json") ([ordered]@{
    ...
    question = "Does the shared Reactor Netty pool reach maxConnections=500 with pending acquisition and direct pending-limit failure under HIGH load?"
})
...
if ($ExecutionMode -eq "PREFLIGHT") {
    Write-Output "EXP142_STATIC_PREFLIGHT_PASS"
    exit 0
}
...
try {
    ...
    $warmup = Invoke-NodeLoad -RunId "WARMUP-EXP142-POOL500-METER-001" -ActiveMissions 1 `
    ...
    if ($null -eq $poolReady) {
        ...
        throw "EXP142_POOL500_METER_NOT_READY_AFTER_WARMUP"
    }
    ...
    $high = Invoke-NodeLoad -RunId "RUN-EXP142-POOL500-HIGH-001" -ActiveMissions 200 `
    ...
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP142_POOL500_PROBE_COMPLETE"
        warmupRunId = "WARMUP-EXP142-POOL500-METER-001"
        highRunId = "RUN-EXP142-POOL500-HIGH-001"
        additionalRuns = 0
        pool500Executed = $true
        mvcExecuted = $false
        productionJavaModified = $false
    })
    Write-Output "EXP142_POOL500_PROBE_COMPLETE"
} catch {
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP142_EXECUTION_STOPPED"
