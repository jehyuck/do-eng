...
$firstActiveLimit = First-AtOrAbove $active 599.5
...
$contractValid = ($configuredMaxConnections -eq 600 -and $configuredMaxPending -eq 800)
...
} elseif ($meterCoverageValid -and $peakActive -ge 599.5 -and $peakPending -gt 0 -and $exceptionCounts.poolAcquirePendingLimit -gt 0) {
    $classification = "POOL600_RUNTIME_SATURATION_PROVEN"
} elseif ($meterCoverageValid -and $peakActive -lt 599.5) {
    $classification = "POOL600_RUNTIME_NOT_SATURATED"
} else {
    $classification = "INSUFFICIENT_POOL600_EVIDENCE"
}
...
$result = [ordered]@{
    ...
    interpretationGuard = @(
        ...
        "POOL600_RUNTIME_SATURATION_PROVEN requires active-at-limit, positive pending, and PoolAcquirePendingLimitException together",
...
- First active >= 600: `$($firstActiveLimit.timestamp)`
