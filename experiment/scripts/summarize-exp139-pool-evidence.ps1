...
$firstActiveLimit = First-AtOrAbove $active 399.5
...
$contractValid = ($configuredMaxConnections -eq 400 -and $configuredMaxPending -eq 800)
...
} elseif ($meterCoverageValid -and $peakActive -ge 399.5 -and $peakPending -gt 0 -and $exceptionCounts.poolAcquirePendingLimit -gt 0) {
    $classification = "CONNECTION_POOL_400_SATURATION_PROVEN"
} elseif ($meterCoverageValid -and $peakActive -ge 399.5 -and $peakPending -gt 0 -and ($http500 + $clientTimeout) -gt 0) {
    $classification = "CONNECTION_POOL_PRESSURE_HIGH_CONFIDENCE"
} elseif ($meterCoverageValid -and $peakActive -lt 360 -and $peakPending -eq 0) {
    $classification = "POOL_NOT_DOMINANT"
}
...
$result = [ordered]@{
    ...
    interpretationGuard = @(
        ...
        "CONNECTION_POOL_400_SATURATION_PROVEN requires active-at-limit, positive pending, and PoolAcquirePendingLimitException together",
...
- First active >= 400: `$($firstActiveLimit.timestamp)`
