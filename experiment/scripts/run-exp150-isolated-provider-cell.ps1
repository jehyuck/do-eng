...
$overrideSource = Join-Path $repo "experiment\compose\experiment-1-50-factor-b-isolated.override.yml"
$overrideCompose = Join-Path $env:TEMP ("doeng-exp150-" + $ObservationMode.ToLowerInvariant() + ".override.yml")
...
$summarizer = Join-Path $repo "experiment\scripts\summarize-exp150-isolated-provider.ps1"
$fixture = Join-Path $repo "image\arc.jpg"
$resultRoot = Join-Path $repo ("backend\experiments\results\experiment-1-50\isolated-provider\" + ($(if ($ObservationMode -eq "P") { "a1b1-performance-001" } else { "a1b1-diagnostic-001" })))
$planDir = Join-Path $resultRoot "plan"
$warmupDir = Join-Path $resultRoot ("WARMUP\" + ($(if ($ObservationMode -eq "P") { "WARMUP-EXP150-A1B1-P-001" } else { "WARMUP-EXP150-A1B1-D-001" })))
$highDir = Join-Path $resultRoot ("HIGH\" + ($(if ($ObservationMode -eq "P") { "RUN-EXP150-A1B1-PERFORMANCE-001" } else { "RUN-EXP150-A1B1-DIAGNOSTIC-001" })))
$analysisDir = Join-Path $resultRoot "analysis"
$runtimeCompose = $null
$runtimeDbDumpDir = $null
$runtimeSeedImage = $null
$project = if ($ObservationMode -eq "P") { "doeng-exp150-p" } else { "doeng-exp150-d" }
...
if (-not (Test-Path -LiteralPath $overrideSource)) { throw "Exp150 isolated override not found" }
...
function Prepare-RuntimeCompose {
    ...
    $script:runtimeDbDumpDir = Join-Path $env:TEMP ("doeng-exp150-db-seed-" + $ObservationMode.ToLowerInvariant())
    ...
    $script:runtimeSeedImage = "doeng-exp150-mariadb-seeded-$($ObservationMode.ToLowerInvariant()):latest"
    ...
    $script:runtimeCompose = Join-Path $repo ("backend\docker-compose.experiment.exp150-runtime-$($ObservationMode.ToLowerInvariant()).yaml")
...
if (-not (Test-Path -LiteralPath $summarizer)) { throw "Exp150 isolated-provider summarizer not found" }
...
if ($dirty.Count -gt 0) {
    $allowedResultPrefix = ((Resolve-Path (Join-Path $repo "backend\experiments\results\experiment-1-50") -ErrorAction SilentlyContinue).Path)
    if ([string]::IsNullOrWhiteSpace($allowedResultPrefix)) {
        throw "Working tree must be clean outside the Exp150 result root."
    }
...
$requiredPatterns = [ordered]@{
    poolMode = 'DOENG_EXTERNAL_POOL_MODE:\s*"?ISOLATED"?'
    tokenMax = 'DOENG_TOKEN_POOL_MAX_CONNECTIONS:\s*"?50"?'
    tokenPending = 'DOENG_TOKEN_POOL_PENDING_MAX_COUNT:\s*"?80"?'
    aiMax = 'DOENG_AI_POOL_MAX_CONNECTIONS:\s*"?400"?'
    aiPending = 'DOENG_AI_POOL_PENDING_MAX_COUNT:\s*"?640"?'
    storageMax = 'DOENG_STORAGE_POOL_MAX_CONNECTIONS:\s*"?50"?'
    storagePending = 'DOENG_STORAGE_POOL_PENDING_MAX_COUNT:\s*"?80"?'
...
Save-Json (Join-Path $planDir "static-contract.json") ([ordered]@{
    status = "PASS"
    poolMode = "DOENG_EXTERNAL_POOL_MODE=ISOLATED"
    tokenPool = "50/80"
    aiPool = "400/640"
    storagePool = "50/80"
...
try {
    ...
    $runtimeContract = [ordered]@{
        ...
        tokenMax = $appEnv.DOENG_TOKEN_POOL_MAX_CONNECTIONS
        tokenPending = $appEnv.DOENG_TOKEN_POOL_PENDING_MAX_COUNT
        aiMax = $appEnv.DOENG_AI_POOL_MAX_CONNECTIONS
        aiPending = $appEnv.DOENG_AI_POOL_PENDING_MAX_COUNT
        storageMax = $appEnv.DOENG_STORAGE_POOL_MAX_CONNECTIONS
        storagePending = $appEnv.DOENG_STORAGE_POOL_PENDING_MAX_COUNT
    ...
    if ($runtimeContract.poolMode -ne "ISOLATED" -or
        $runtimeContract.tokenMax -ne "50" -or $runtimeContract.tokenPending -ne "80" -or
        $runtimeContract.aiMax -ne "400" -or $runtimeContract.aiPending -ne "640" -or
        $runtimeContract.storageMax -ne "50" -or $runtimeContract.storagePending -ne "80") { throw "Pool runtime env mismatch" }
    ...
    if ($ObservationMode -eq "D") {
        try {
            ...
            throw "EXP150_STAGE_ENDPOINT_INVALID: $($_.Exception.Message)"
        }
    }

    $warmupRunId = if ($ObservationMode -eq "P") { "WARMUP-EXP150-A1B1-P-001" } else { "WARMUP-EXP150-A1B1-D-001" }
    $highRunId = if ($ObservationMode -eq "P") { "RUN-EXP150-A1B1-PERFORMANCE-001" } else { "RUN-EXP150-A1B1-DIAGNOSTIC-001" }
    ...
    for ($attempt = 1; $attempt -le 10; $attempt++) {
        try {
            ...
            foreach ($provider in $providers) {
                $providerReadiness[$provider] = [ordered]@{
                    active = Metric-Value $snapshot "reactor.netty.connection.provider.active.connections" $provider
                    pending = Metric-Value $snapshot "reactor.netty.connection.provider.pending.connections" $provider
                    maxConnections = Metric-Value $snapshot "reactor.netty.connection.provider.max.connections" $provider
    ...
    if ($null -eq $poolReady) {
        ...
        throw "EXP150_ISOLATED_PROVIDER_METERS_NOT_READY_AFTER_WARMUP"
    }
    ...
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = if ($ObservationMode -eq "P") { "EXP150_A1B1_P_COMPLETE" } else { "EXP150_A1B1_D_COMPLETE" }
    ...
    $completionMarker = if ($ObservationMode -eq "P") { "EXP150_A1B1_P_COMPLETE" } else { "EXP150_A1B1_D_COMPLETE" }
    Write-Output $completionMarker
} catch {
    Save-Json (Join-Path $planDir "execution-status.json") ([ordered]@{
        status = "EXP150_EXECUTION_STOPPED"
