...
$firstPendingLimit = First-AtOrAbove $pending 1
...
$successfulRps = if ($attempts -gt 0) { [double]$http200 / 30.0 } else { $null }
$uncontrolledFailureRate = if ($attempts -gt 0) { [double]($http500 + $clientTimeout + $connectionError) / $attempts } else { $null }

$contractValid = ($configuredMaxConnections -eq 500 -and $configuredMaxPending -eq 1)
...
$classification = "INSUFFICIENT_PENDING1_EVIDENCE"
if (-not $contractValid) {
    $classification = "INVALID_PENDING1_CONTRACT"
} elseif (-not $meterCoverageValid) {
    $classification = "INSUFFICIENT_PENDING1_EVIDENCE"
} elseif ($successfulRps -gt 108.02 -and $clientTimeout -lt 855 -and $uncontrolledFailureRate -lt 0.5045) {
    $classification = "PENDING1_CAPACITY_IMPROVED"
} elseif ($peakActive -ge 499.5 -and $exceptionCounts.poolAcquirePendingLimit -gt 3922 -and $clientTimeout -lt 855 -and $successfulRps -ge 88.38 -and $successfulRps -lt 108.02) {
    $classification = "PENDING1_FAST_REJECTION_CONFIRMED"
} elseif ($clientTimeout -ge 855 -or $successfulRps -lt 88.38) {
    $classification = "PENDING1_TIMEOUT_PERSISTS"
} else {
    $classification = "INSUFFICIENT_PENDING1_EVIDENCE"
}
...
$result = [ordered]@{
    ...
    poolContract = [ordered]@{
        ...
        expectedMaxPendingConnections = 1
    ...
    comparison = [ordered]@{
        baselineSuccessfulRps = 98.20
        baselineClientTimeout = 855
        baselineUncontrolledFailureRate = 0.5045
        successfulRps = $successfulRps
        uncontrolledFailureRate = $uncontrolledFailureRate
    }
    interpretationGuard = @(
        ...
        "PENDING1_FAST_REJECTION_CONFIRMED requires active-at-limit, pending-limit failures above the Pool500/Pending800 baseline, reduced client timeouts, and successful RPS within +/-10% of baseline",
...
$markdown = @"
# Exp145 Pool500 Pending1 Diagnostic Summary
...
- First pending >= 1: `$($firstPending.timestamp)`
...
- Uncontrolled failure rate: `$uncontrolledFailureRate`
- HTTP 500: `$http500`
