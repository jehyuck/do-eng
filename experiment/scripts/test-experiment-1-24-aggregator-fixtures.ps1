...
$fixture=Join-Path $repo 'backend\experiments\results\experiment-1-24\fixture-aggregator'
...
function New-ValidRun([string]$arm){
 $id="RUN-20990101-EXP124-$arm";$condition=if($arm.StartsWith('BASELINE')){'BASELINE'}else{'REMEDIATION'};$dir=Join-Path $core $id;$app=Join-Path $dir 'application';$pool=Join-Path $dir 'pool';$prov=Join-Path $dir 'provenance'
...
$dockerArgs=@('run','--rm','--mount',"type=bind,source=$repo,target=/repo",'--workdir','/repo','node:24.18.0-bookworm-slim','node','experiment/scripts/aggregate-experiment-1-24.js','--readiness','20990101','backend/experiments/results/experiment-1-24/fixture-aggregator/core','backend/experiments/results/experiment-1-24/fixture-aggregator/aggregate.json')
...
foreach($m in $mutations){$dir=Join-Path $core 'RUN-20990101-EXP124-BASELINE-001';$p=Join-Path $dir $m.path;$backup=Get-Content $p -Raw;$o=$backup|ConvertFrom-Json;$o=& $m.action $o;Write-Json $p $o;if($m.name -eq 'X12'){$summaryPath=$p};if($m.name -eq 'X13' -or $m.name -eq 'X14' -or $m.name -eq 'X15'){$summaryPath=$out};& docker @dockerArgs|Out-Null;$check=Get-Content $out -Raw|ConvertFrom-Json;if($check.aggregateReadiness -ne 'INCOMPLETE'){throw "$($m.name)_MUTATION_NOT_REJECTED"};Set-Content $p $backup -Encoding UTF8}
$runDir=Join-Path $core 'RUN-20990101-EXP124-BASELINE-001';New-Item -ItemType File (Join-Path $runDir 'RUNNING')|Out-Null;& docker @dockerArgs|Out-Null;$check=Get-Content $out -Raw|ConvertFrom-Json;if($check.aggregateReadiness -ne 'INCOMPLETE'){throw 'X18_MUTATION_NOT_REJECTED'};Remove-Item (Join-Path $runDir 'RUNNING') -Force
Write-Output 'EXP124_AGGREGATOR_FIXTURES_X10_X18_PASS'
