...
$dir=Join-Path $root 'backend\experiments\results\experiment-1-28\lifecycle-fixtures';if(Test-Path $dir){throw 'FIXTURE_COLLISION'};New-Item -ItemType Directory -Force (Join-Path $dir 'provenance'),(Join-Path $dir 'pool')|Out-Null
...
$life=New-Exp125CollectorLifecycle 'FIXTURE-EXP128' 'CORE_BOUND_STOP_SIGNAL' 1000 10 10 10 300 $t.a $t.b $t.c $t.d $t.e $t.e $t.f $t.g $true 0 $false $false 7000 'COLLECTOR_LIFECYCLE_PASSED' $null 'COLLECTOR_STOPPED_BY_SIGNAL' 0 $true;if([string]::IsNullOrWhiteSpace([string]$life.failureType)){$life.failureType=$null}
...
$life.failureType='';$life|ConvertTo-Json|Set-Content (Join-Path $dir 'pool/collector-lifecycle.json') -Encoding UTF8;$rejected=$false;try{Assert-Exp125CollectorLifecycle $dir}catch{$rejected=$true};if(-not$rejected){throw 'L4_FAILED'};Write-Output 'EXP128_LIFECYCLE_NULL_REGRESSION_PASS'
